...
export default function  RecipeList({item}){
    ...
    return <div className="flex flex-col w-80 overflow-hidden p-5 bg-white/75 shadow-xl gap-5 border-2 rounded-2xl border-white">
        ...
        <div>
            ...
            <Link to={`/reciepe-item/${item?.id}`} className="text-sm p-3 px-0 mt-5 rounded-lg uppercase font-medium tracking-wider inline-block shadow-md bg-black text-white">Recipe Details</Link>
        </div>
    </div>
}